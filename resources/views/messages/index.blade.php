    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @if($useOwnerNavbar)
            ...
            @include('owner.partials.owner-messages-index-styles')
...
<body class="{{ $useOwnerNavbar ? 'owner-nav-page' : ($authUser?->isClient() ? 'client-nav-page font-sans text-gray-800' : 'min-h-screen bg-gradient-to-br from-green-50 via-lime-50 to-white text-gray-800') }}">
    ...
    <main class="messages-index-main client-guest-main client-guest-main--full guest-messages-main">
        ...
        @include('messages.partials.messages-inbox-split', [
            'surfacePrefix' => 'guest-messages',
            'emptyStateHint' => 'Start one with the owner or an administrator.',
        ])
    </main>
    @elseif($useOwnerNavbar)
    <main class="messages-index-main main-content with-owner-nav owner-app-main owner-messages-main">
        <header class="owner-messages-top owner-page-top">
            ...
            @if($showComposeButton)
                <a href="{{ route('messages.create', [], false) }}" class="owner-messages-hero__cta">
        ...
        @include('messages.partials.messages-inbox-split', [
            'surfacePrefix' => 'owner-messages',
            'emptyStateHint' => 'Start one with a guest, a team member, or ImpaStay central support.',
        ])
    </main>
    @else
    <main
        class="messages-index-main mx-auto flex min-h-screen w-full max-w-none flex-col px-3 pb-6 sm:px-4 lg:px-6"
        @if(! $authUser?->isClient()) style="padding-top: calc(var(--client-nav-offset) + 12px);" @endif
    >
